# 2026-United-States-Midterm-Election-Model

The 2026 U.S. Midterm Elections on November 3rd, 2026 will occur during Donald Trump's second term, with all 435 House seats, 33 regularly scheduled Senate seats alongside two special election Senate seats, and 39 gubernatorial races.

### Sources

#### Program

-   [R](https://www.r-project.org)
-   [VS Code](https://code.visualstudio.com)

#### Data

-   [U.S. Congress GIS Database](https://simplemaps.com/data/congress)
-   [FiveThirtyEight](https://abcnews.go.com/538)
-   [VoteHub Polling API](https://portal.votehub.com)
-   [Wikipedia - 2024 United States House of Representatives Elections](https://en.wikipedia.org/wiki/2024_United_States_House_of_Representatives_elections)

#### Tools

-   [wikitable2csv](https://wikitable2csv.ggor.de)

#### Packages

-   [dplyr](https://dplyr.tidyverse.org)
-   [ggplot2](https://ggplot2.tidyverse.org)
-   [ggthemes](https://yutani.rbind.io/ggthemes/)
-   [tidyverse](https://www.tidyverse.org)
-   [tidyr](https://tidyr.tidyverse.org)
-   [readxl](https://readxl.tidyverse.org)
-   [sf](https://r-spatial.github.io/sf/)
-   [rmapshaper](https://cran.r-project.org/web/packages/rmapshaper/index.html)
-   [USAboundaries](https://ropensci.org/packages/usaboundaries/)
-   [httr](https://httr.r-lib.org)
-   [jsonlite](https://cran.r-project.org/web/packages/jsonlite/index.html)
-   [scales](https://scales.r-lib.org)
-   [DT](https://rstudio.github.io/DT/)
-   [ggnewscale](https://cran.r-project.org/web/packages/ggnewscale/index.html)