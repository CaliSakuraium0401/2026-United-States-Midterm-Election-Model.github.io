...
us_states <- us_states()
us_congressional_2022 <- as_tibble(st_read(file.path("spacial/congressional_districts.shp")))
```
  ...
  mutate(
    ...
    seat_number = ifelse(
      ...
      ifelse(
        state_abbr == "AK", 
        "99", 
        sprintf("%02d", as.integer(seat_number))  # Add leading zeros for numbers < 10
      )
    ),
...
house_polling_sf <- house_polling_aggregated |>
  left_join(
    us_congressional_2022 |> select(id, geometry),
    join_by(district_name  == id)
  )
```
...
ggplot() +
    ...
    geom_sf(
        ...
        aes(geometry = geometry, fill=DEM_LEAD_PCT),
        show.legend = FALSE
    ) +
    scale_fill_gradient2(high = "#002B84", mid = '#B9D7FF', low = "#800000", midpoint = 0) +
    labs(
